...
```python
# Summary statistics
crashes.describe()       # basic stats for numeric columns

# Specific column
crashes['casualty_age'].mean()
crashes['casualty_age'].median()
crashes['casualty_age'].std()
```
...
## Ordering Data with sort_values()
...
```python
# Write to CSV
result.to_csv("outputs/processed_crashes.csv", index=False)
```
...
## Beyond Pandas: Polars and DuckDB

For large datasets or high-performance data processing:

-   **[Polars](https://docs.pola.rs/):** A modern, lightning-fast DataFrame library written in Rust with parallel processing and an expressive query syntax.
-   **[DuckDB](https://duckdb.org/):** An embedded analytical SQL database ("SQLite for analytics") capable of querying millions of rows directly from CSV or Parquet files.
