...
path_to_file <- "data/RunningData.xlsx"
...
data_adults |> 
  ggplot(aes(x = time))+
  geom_histogram(binwidth = 1, col = "white", fill = "steelblue')  # ERROR 1
...
data_adults_lm <- data_adults |> 
  mutate(age = str_extract(age_cat, '^\\d{2}") |> as.numeric())  # ERROR 4

# Building a linear model
my_linear_model <- lm(formula = "time ~ age + gender + prev_runs", dta = data_adults_lm)  # ERROR 5
